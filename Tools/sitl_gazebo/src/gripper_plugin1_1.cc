#ifndef _GRIPPER_PLUGIN1_1_HH_
#define _GRIPPER_PLUGIN1_1_HH_
...
namespace gazebo
{
	...
	class GripperROSPlugin1_1 : public ModelPlugin
	{
    		/// \brief Constructor
		public: GripperROSPlugin1_1() {}
...
//        		_model->GetJointCount() << "]\n";
			...
			if (!ros::isInitialized())
			{
  				int argc = 0;
  				char **argv = NULL;
  				ros::init(argc, argv, "gazebo_manipulator_client_gripper1_1",
      				ros::init_options::NoSigintHandler);
			}

			// Create our ROS node. This acts in a similar manner to
			// the Gazebo node
			this->rosNode.reset(new ros::NodeHandle("gazebo_manipulator_client_gripper1_1"));
			ros::NodeHandle n;
 			this->posi_pub = n.advertise<std_msgs::Float32>("joint/gripper1_1/position_msg", 10);
			// Create a named topic, and subscribe to it.
			ros::SubscribeOptions so =
  			ros::SubscribeOptions::create<std_msgs::Float32>(
      			"/joint/gripper1_1/position_cmd",
      			0,
      			boost::bind(&GripperROSPlugin1_1::OnRosMsg, this, _1),
      			ros::VoidPtr(), &this->rosQueue);
			this->rosSub = this->rosNode->subscribe(so);

			// Spin up the queue helper thread.
			this->rosQueueThread =
  			std::thread(std::bind(&GripperROSPlugin1_1::QueueThread, this));
  			this->model->GetJointController()->SetPositionTarget(
	...
	// Tell Gazebo about this plugin, so that Gazebo can call Load on this plugin.
 	GZ_REGISTER_MODEL_PLUGIN(GripperROSPlugin1_1)
}
#endif
