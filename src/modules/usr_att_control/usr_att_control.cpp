...
void UserAttitudeControl::parameters_updated()
{
    ...
    _attitude_control.setControllerGain(Vector3f(_param_usr_lambda_q_x.get(), _param_usr_lambda_q_y.get(), _param_usr_lambda_q_z.get()),
                                  Vector3f(_param_usr_k_omega_x.get(), _param_usr_k_omega_x.get(), _param_usr_k_omega_x.get()),
                                  Vector3f(_param_usr_eso_gain_x.get(), _param_usr_eso_gain_x.get(), _param_usr_eso_gain_x.get()),
                                  _param_usr_Kq.get());
...
//        if (run_att_ctrl || manual_has_been_changed_to_att) {
        if (run_att_ctrl) {
            ...
            // TODO: add tau_static item
            _mavros_gs_sub.copy(&_gs_for_statical); // 获取重心位置
            Vector3f tau_static(_gs_for_statical.gs);
            Dcmf R_e2b(q);
            // tau_static =(_mass_uav+_mass_manip)*(tau_static%(R_e2b.transpose()*Vector3f(0,0,9.81)));
            tau_static =(_mass_manip)*(tau_static%(R_e2b.transpose()*Vector3f(0,0,9.81)));
            Vector3f rates_sp;
           _attitude_control.update(q,rates, tau_static,dt, _landed, _torque,rates_sp);
