 ...
 *Author: Huazi Cao
 *Derived from module mc_pos_control
 ****************************************************************************/

#include "UserPositionControl.hpp"
...
UserPositionControl::UserPositionControl(bool vtol) :
...
UserPositionControl::~UserPositionControl()
{
	perf_free(_cycle_perf);
}

bool UserPositionControl::init()
{
...
int UserPositionControl::parameters_update(bool force)
{
	...
	if (_parameter_update_sub.updated() || force) {
		...
		int num_changed = 0;
                ...
                if (_param_usr_vehicle_resp.get() >= 0.f) {
			// make it less sensitive at the lower end
                        float responsiveness = _param_usr_vehicle_resp.get() * _param_usr_vehicle_resp.get();

                        num_changed += _param_usr_acc_hor.commit_no_notification(math::lerp(1.f, 15.f, responsiveness));
                        num_changed += _param_usr_acc_hor_max.commit_no_notification(math::lerp(2.f, 15.f, responsiveness));
                        num_changed += _param_usr_man_y_max.commit_no_notification(math::lerp(80.f, 450.f, responsiveness));

			if (responsiveness > 0.6f) {
                                num_changed += _param_usr_man_y_tau.commit_no_notification(0.f);

			} else {
                                num_changed += _param_usr_man_y_tau.commit_no_notification(math::lerp(0.5f, 0.f, responsiveness / 0.6f));
			}

			if (responsiveness < 0.5f) {
                                num_changed += _param_usr_tiltmax_air.commit_no_notification(45.f);

			} else {
                                num_changed += _param_usr_tiltmax_air.commit_no_notification(math::min(MAX_SAFE_TILT_DEG, math::lerp(45.f, 70.f,
						(responsiveness - 0.5f) * 2.f)));
			}

                        num_changed += _param_usr_acc_down_max.commit_no_notification(math::lerp(0.8f, 15.f, responsiveness));
                        num_changed += _param_usr_acc_up_max.commit_no_notification(math::lerp(1.f, 15.f, responsiveness));
                        num_changed += _param_usr_jerk_max.commit_no_notification(math::lerp(2.f, 50.f, responsiveness));
                        num_changed += _param_usr_jerk_auto.commit_no_notification(math::lerp(1.f, 25.f, responsiveness));
		}

                if (_param_usr_xy_vel_all.get() >= 0.f) {
                        float xy_vel = _param_usr_xy_vel_all.get();
                        num_changed += _param_usr_vel_manual.commit_no_notification(xy_vel);
                        num_changed += _param_usr_xy_cruise.commit_no_notification(xy_vel);
                        num_changed += _param_usr_xy_vel_max.commit_no_notification(xy_vel);
		}

                if (_param_usr_z_vel_all.get() >= 0.f) {
                        float z_vel = _param_usr_z_vel_all.get();
                        num_changed += _param_usr_z_vel_max_up.commit_no_notification(z_vel);
                        num_changed += _param_usr_z_vel_max_dn.commit_no_notification(z_vel * 0.75f);
                        num_changed += _param_usr_tko_speed.commit_no_notification(z_vel * 0.6f);
                        num_changed += _param_usr_land_speed.commit_no_notification(z_vel * 0.5f);
		}
		...
		if (num_changed > 0) {
			param_notify_changes();
                ...
                if (_param_usr_tiltmax_air.get() > MAX_SAFE_TILT_DEG) {
                        _param_usr_tiltmax_air.set(MAX_SAFE_TILT_DEG);
                        _param_usr_tiltmax_air.commit();
			mavlink_log_critical(&_mavlink_log_pub, "Tilt constrained to safe value");
		}

                if (_param_usr_tiltmax_lnd.get() > _param_usr_tiltmax_air.get()) {
                        _param_usr_tiltmax_lnd.set(_param_usr_tiltmax_air.get());
                        _param_usr_tiltmax_lnd.commit();
			mavlink_log_critical(&_mavlink_log_pub, "Land tilt has been constrained by max tilt");
		}
                _control.setMasses(_param_usr_mass_uav.get(), _param_usr_mass_manipulator.get());
                _control.setControlParas(
                            Vector3f(_param_usr_lambda_p_x.get(), _param_usr_lambda_p_y.get(), _param_usr_lambda_p_z.get()),
                            Vector3f(_param_usr_k_v_x.get(), _param_usr_k_v_y.get(), _param_usr_k_v_z.get()));
		 _control.setESOParas(Vector3f(_param_eso_v_x.get(), _param_eso_v_y.get(), _param_eso_v_z.get()));

		// Check that the design parameters are inside the absolute maximum constraints
                if (_param_usr_xy_cruise.get() > _param_usr_xy_vel_max.get()) {
                        _param_usr_xy_cruise.set(_param_usr_xy_vel_max.get());
                        _param_usr_xy_cruise.commit();
			mavlink_log_critical(&_mavlink_log_pub, "Cruise speed has been constrained by max speed");
		}

                if (_param_usr_vel_manual.get() > _param_usr_xy_vel_max.get()) {
                        _param_usr_vel_manual.set(_param_usr_xy_vel_max.get());
                        _param_usr_vel_manual.commit();
			mavlink_log_critical(&_mavlink_log_pub, "Manual speed has been constrained by max speed");
		}

                if (_param_mpc_thr_hover.get() > _param_usr_thr_max.get() ||
                    _param_mpc_thr_hover.get() < _param_usr_thr_min.get()) {
                        _param_mpc_thr_hover.set(math::constrain(_param_mpc_thr_hover.get(), _param_usr_thr_min.get(),
                                                 _param_usr_thr_max.get()));
                        _param_mpc_thr_hover.commit();
			mavlink_log_critical(&_mavlink_log_pub, "Hover thrust has been constrained by min/max");
		}

                if (!_param_usr_use_hte.get() || !_hover_thrust_initialized) {
                        _control.setHoverThrust(_param_mpc_thr_hover.get());
			_hover_thrust_initialized = true;
		}

		// initialize vectors from params and enforce constraints
                _param_usr_tko_speed.set(math::min(_param_usr_tko_speed.get(), _param_usr_z_vel_max_up.get()));
                _param_usr_land_speed.set(math::min(_param_usr_land_speed.get(), _param_usr_z_vel_max_dn.get()));

                _takeoff.setSpoolupTime(_param_usr_spoolup_time.get());
                _takeoff.setTakeoffRampTime(_param_usr_tko_ramp_t.get());
                _takeoff.generateInitialRampValue(_param_usr_k_v_z.get());
	}

	return OK;
}

PositionControlStates UserPositionControl::set_vehicle_states(const vehicle_local_position_s &local_pos)
{
	...
	if (_local_pos_sub.update(&local_pos)) {
		...
		PositionControlStates states{set_vehicle_states(local_pos)};
			...
			// TODO: this should get obsolete once the takeoff limiting moves into the flight tasks
                        if (!PX4_ISFINITE(_vehicle_constraints.speed_up) || (_vehicle_constraints.speed_up > _param_usr_z_vel_max_up.get())) {
                                _vehicle_constraints.speed_up = _param_usr_z_vel_max_up.get();
			}
				...
				// override with defaults
                                _vehicle_constraints.speed_xy = _param_usr_xy_vel_max.get();
                                _vehicle_constraints.speed_up = _param_usr_z_vel_max_up.get();
                                _vehicle_constraints.speed_down = _param_usr_z_vel_max_dn.get();
			}
			...
			const float tilt_limit_deg = (_takeoff.getTakeoffState() < TakeoffState::flight)
                                                     ? _param_usr_tiltmax_lnd.get() : _param_usr_tiltmax_air.get();
			...
			const float speed_up = _takeoff.updateRamp(dt,
                                               PX4_ISFINITE(_vehicle_constraints.speed_up) ? _vehicle_constraints.speed_up : _param_usr_z_vel_max_up.get());
			const float speed_down = PX4_ISFINITE(_vehicle_constraints.speed_down) ? _vehicle_constraints.speed_down :
                                                 _param_usr_z_vel_max_dn.get();
			const float speed_horizontal = PX4_ISFINITE(_vehicle_constraints.speed_xy) ? _vehicle_constraints.speed_xy :
                                                       _param_usr_xy_vel_max.get();

			// Allow ramping from zero thrust on takeoff
                        const float minimum_thrust = flying ? _param_usr_thr_min.get() : 0.f;

                        _control.setThrustLimits(minimum_thrust, _param_usr_thr_max.get());

			_control.setVelocityLimits(
                                math::constrain(speed_horizontal, 0.f, _param_usr_xy_vel_max.get()),
                                math::min(speed_up, _param_usr_z_vel_max_up.get()), // takeoff ramp starts with negative velocity limit
                                math::constrain(speed_down, 0.f, _param_usr_z_vel_max_dn.get()));
			...
			if (!PX4_ISFINITE(_setpoint.z)
				...
				//  >= MPC_LAND_SPEED: use altitude derivative
                                float weighting = fminf(fabsf(_setpoint.vz) / _param_usr_land_speed.get(), 1.f);
			...
			if (_control.update(dt)) {
				...
				_control.setInputSetpoint(failsafe_setpoint);
                                _control.setVelocityLimits(_param_usr_xy_vel_max.get(), _param_usr_z_vel_max_up.get(), _param_usr_z_vel_max_dn.get());
				_control.update(dt);
			}
...
void UserPositionControl::failsafe(const hrt_abstime &now, vehicle_local_position_setpoint_s &setpoint,
	...
	if (_failsafe_land_hysteresis.get_state()) {
		...
		if (PX4_ISFINITE(states.velocity(0)) && PX4_ISFINITE(states.velocity(1))) {
			...
			setpoint.acceleration[0] = setpoint.acceleration[1] = 0.f;
                        setpoint.vz = _param_usr_land_speed.get();
...
void UserPositionControl::reset_setpoint_to_nan(vehicle_local_position_setpoint_s &setpoint)
{
...
int UserPositionControl::task_spawn(int argc, char *argv[])
{
	bool vtol = false;
	...
	if (argc > 1) {
        ...
        UserPositionControl *instance = new UserPositionControl(vtol);
...
int UserPositionControl::custom_command(int argc, char *argv[])
{
	return print_usage("unknown command");
}

int UserPositionControl::print_usage(const char *reason)
{
	if (reason) {
		PX4_WARN("%s\n", reason);
	}
...
extern "C" __EXPORT int usr_pos_control_main(int argc, char *argv[])
{
        return UserPositionControl::main(argc, argv);
}
